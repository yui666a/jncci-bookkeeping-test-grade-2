...
# 原本は「資産・負債・純資産・収益・費用」に加えて「その他」の区分を持つ。
# これを見出しとして認識しないと、直前の「費用」が末尾まで引き継がれ、
# 損益・支店・本店・未決算・保証債務が費用に分類される。
CATEGORY = re.compile(r'^(資産|負債|純資産(?:（資本）)?|収益|費用|その他)$')
...
def main():
    ...
    lines = ['# 商業簿記標準・許容勘定科目表（2022年度版）を構造化したもの。',
             ...
             '# 工業簿記の科目はこの表で照合できない。',
             '#',
             '# category は信用しない。原本の見出しを直前から引き継ぐ実装のため、',
             '# 見出しが変わらないまま続く箇所で誤った分類が付く',
             '# （未決算・保証債務・保証債務見返などが「費用」になる）。',
             '# この表を根拠にしてよいのは name と grade だけである。',
             'accounts:']
