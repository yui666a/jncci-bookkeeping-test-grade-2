...
- **`mount()` 先の要素には必ず `id` を付ける。** id がないと学習記録の
  保存先が決まらず、その設問は永久に記録されない（ゲート9が落とす）
- **全選択肢が金額を含む `BokiQuiz` には `invariant` を書く**（ゲート10が落とす）。
  記法は `教材制作ルール.md` の「BokiQuiz の数値検算」
- 本文の最後、`<div class="footer">` の直前に `<div data-note></div>` を置く。
  学習者がその場で疑問を書き残すためのメモ欄になる
